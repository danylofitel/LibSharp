...
public class AsyncLockUnitTests
{
    ...
    public async Task AcquireAsync_ThrowsWhenDisposed()
    {
        ...
        _ = await Assert.ThrowsExactlyAsync<ObjectDisposedException>(async () =>
            await asyncLock.AcquireAsync(TestContext.CancellationToken).ConfigureAwait(false)).ConfigureAwait(false);
    }
    ...
    public async Task AcquireAsync_HandleDispose_ReleasesLock()
    {
        ...
        using (AsyncLock asyncLock = new AsyncLock())
        {
            using (await asyncLock.AcquireAsync(TestContext.CancellationToken).ConfigureAwait(false))
            {
                // first acquisition
            }
    ...
    public async Task Dispose_WithManyPendingWaiters_AllWaitersUnblocked()
    {
        // Arrange
        AsyncLock asyncLock = new AsyncLock();
        AsyncLock.Handle holder = await asyncLock.AcquireAsync(TestContext.CancellationToken).ConfigureAwait(false);
    ...
    public async Task Handle_DoubleDispose_DoesNotThrow()
    {
        ...
        using (AsyncLock asyncLock = new AsyncLock())
        {
            AsyncLock.Handle handle = await asyncLock.AcquireAsync(TestContext.CancellationToken).ConfigureAwait(false);
    ...
    public async Task Handle_DoubleDispose_DoesNotReleaseExtraPermit()
    {
        ...
        using (AsyncLock asyncLock = new AsyncLock())
        {
            AsyncLock.Handle firstHandle = await asyncLock.AcquireAsync(TestContext.CancellationToken).ConfigureAwait(false);
            firstHandle.Dispose();
    ...
    public async Task Handle_CopiedStruct_DisposeFromBothCopies_ReleasesOnlyOnce()
    {
        ...
        using (AsyncLock asyncLock = new AsyncLock())
        {
            AsyncLock.Handle originalHandle = await asyncLock.AcquireAsync(TestContext.CancellationToken).ConfigureAwait(false);
    ...
    public async Task AcquireAsync_DisposeRace_DoesNotSurfaceOperationCanceledException()
    {
        ...
        for (int i = 0; i < 400; i++)
        {
            ...
            Task<AsyncLock.Handle> acquireTask = Task.Run(async () =>
                await asyncLock.AcquireAsync(TestContext.CancellationToken).ConfigureAwait(false));
    ...
    public async Task Dispose_IdempotentMultipleCalls_DoesNotThrow()
    {
        // Arrange
        AsyncLock asyncLock = new AsyncLock();
        _ = await asyncLock.AcquireAsync(TestContext.CancellationToken).ConfigureAwait(false);
