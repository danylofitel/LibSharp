...
- 5.0.0
  ...
  - `Collections`
    ...
    - `ConcurrentHashSet<T>` now documents that its query operations are non-atomic as well as its compound mutations. `SetEquals`, `IsProperSubsetOf` and `IsProperSupersetOf` read `Count` and walk the elements as separate steps, so under concurrent mutation they can return an answer that was never true at any single moment. `Count` also takes every bucket lock
  ...
  - `Common`
    ...
    - `StringExtensions.Reverse` now builds its result with a single allocation instead of one string per grapheme cluster plus a concatenation
    - `Optional<T>.ToString()` and `Result<T, TError>.ToString()` now document that they cannot distinguish an empty optional from one holding `null`, nor a null success value from a null error, and point at `HasValue` / `IsSuccess` instead
    ...
    - `XmlSerializationExtensions.DeserializeFromXml<T>` now throws `InvalidOperationException` when the XML carries no value, such as an element marked `xsi:nil`. `XmlSerializer.Deserialize` returns null for such input, which the non-nullable return type passed straight to the caller for a reference-typed `T`
  - `Threading`
