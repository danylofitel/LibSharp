...
- 5.0.0
  - `Caching`
    ...
    - Fixed the background refresh loop's initial-fetch phase being subject to the reader-side failure backoff it is documented to be exempt from. The loop paces its own retries; with concurrent reader traffic a reader-recorded failure would suppress the loop's retry and stall it a further retry delay each cycle. The periodic phase was already exempt
    - Fixed an already-cancelled `CancellationToken` throwing from `ProactiveAsyncCache<T>.GetValueAsync` under a stale-read policy. A read served from the stale value returns without waiting, exactly as a fresh hit does, so like a hit it has nothing to cancel. A read that must actually wait still honours the token
    - Documented that the shared-instance check on dropped-value disposal applies to reference types only. A value type is copied per racer, so no test can tell a shared underlying resource from two distinct ones, and disposal there rests on the documented precondition that the factory returns a value it exclusively owns. A factory that cannot meet it should pass `disposeDroppedValues: false`
